...
class Bring:
    ...
    def login(self):
        ...
        try:
            r = requests.post(
                f"{self.url}bringauth",
                data=f"email={self.mail}&password={self.password}",
                timeout=5,
            )
        except:
    ...
    def loadLists(self):
        ...
        try:
            r = requests.get(
                f"{self.url}bringusers/{self.uuid}/lists",
            ...
            us = requests.get(
                f"{self.url}bringusersettings/{self.uuid}",
                headers=self.headers,
                timeout=5,
            ).json()
            ...
            for l in r["lists"]:
                userSettings = next(
                    (obj for obj in uls if obj["listUuid"] == l["listUuid"]), None
                )["usersettings"]
                locale = next(
                    (
                        obj
                        for obj in userSettings
                        if obj["key"] == "listArticleLanguage"
                    ),
                    None,
                )["value"]
    ...
    def getItems(self, _list):
        ...
        try:
            ...
            items = requests.get(
                f"{self.url}bringlists/{listUuid}", headers=self.headers, timeout=5
            ).json()
    ...
    def getAllItemDetails(self, listUuid):
        ...
        try:
            r = requests.get(
                f"{self.url}bringlists/{listUuid}/details",
                headers=self.headers,
                timeout=5,
            )
            return r.json()
    ...
    def saveItem(self, listUuid, itemName, specification=""):
        ...
        try:
            ...
            r = requests.put(
                f"{self.url}bringlists/{listUuid}",
            ...
            print(
                f"Exception: Cannot save item {itemName} ({specification}) to {listUuid}:"
            )
            traceback.print_exc()
            raise
